...
- **Auto-expiración (pg_cron) ACTIVO:** migración `000500` programa `expire_stale_intents()` cada minuto en remoto (jobid 1, corridas OK verificadas). Supervisión vía `cron.job_run_details` (ver runbook). Si no rinde, fallback documentado: ruta cron en Vercel.
- **Ronda 1 de mejoras (ATC-inspired) ✅:** banda CTA dueños B2B en el home (B1), sección FAQ + "Confían en nosotros" (C3), y **amenities** por complejo (C1, migración `000600`, catálogo cerrado de 8: techada/iluminación/estacionamiento/vestuarios/duchas/cafetín/wifi/seguridad) → el dueño las marca en Configuración y se muestran en tarjeta + ficha. Verificado: typecheck 0, build 19 rutas, 20 tests, RPC `public_facilities` con amenities en remoto, smoke `/` y `/c/[slug]` 200. Diferido P2 (ver `BACKLOG.md`): buscador/filtros (A1/A2), reseñas (C2), página B2B con planes (B2).
